...
module.exports = function(app, config) {
    ...
    // auth
    app.post(apiPrefix + '/login', auth.login);  //用户登录
    //app.post(apiPrefix + '/signup', user.checkUser, incode, auth.signup);
    app.post(apiPrefix + '/signup', user.checkUser, auth.signup); //暂时移除邀请码
    app.get(apiPrefix + '/users/:userId/verify', auth.verify);  //邮箱链接激活帐户
